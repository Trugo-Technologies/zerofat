...
public class HangfireService : IJobService
{
    ...
    public string EnqueueAfter(string jobId, Expression<Func<Task>> methodCall) =>
    BackgroundJob.ContinueJobWith(jobId, methodCall);

    public string EnqueueAfter<T>(string jobId, Expression<Action<T>> methodCall) =>
        BackgroundJob.ContinueJobWith(jobId, methodCall);

    public string EnqueueAfter(string jobId, Expression<Action> methodCall) =>
        BackgroundJob.ContinueJobWith(jobId, methodCall);

    public string EnqueueAfter<T>(string jobId, Expression<Func<T, Task>> methodCall) =>
        BackgroundJob.ContinueJobWith(jobId, methodCall);
